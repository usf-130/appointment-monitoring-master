...
import { downloadThisFile, getManagerDocList, getManagerDocListAdmin } from "@/app/services/api/api";
import { useRouter } from "next/navigation";

const DocShow = ({ managerId ,admin}) => {
  ...
  useEffect(() => {
    const getManagerDocListAsync = async () => {
      try {
        if (admin) {
          const response = await getManagerDocListAdmin(managerId);
        setDocs(response.data);
        }else{
          const response = await getManagerDocList(managerId);
          setDocs(response.data);
        }
       
      } catch (error) {
        const elseError = "مشکلی در دریافت مستندات و مدارک مدیر  به وجود آمده است";
        errorHandler(router, error, elseError);
      }
    };

    getManagerDocListAsync();
  }, [managerId]);
