...
import {
  getManagerInitInfoAsAdmin,
  submitManagerInfoAsAdmin,
...
const AddManagerAsAdmin = ({ managerId, updateGridData, selectObjs }) => {
  ...
  useEffect(() => {
    const fetchManagerInfo = async () => {
      try {
        const data = await getManagerInitInfoAsAdmin(managerId);
        setInitValues(data);
      } catch (error) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await submitManagerInfoAsAdmin(dataToSend);
      if (managerId == null) {
  ...
  return (
    ...
    >
      <Row className="my-1">
        ...
        <Col md={8} className="mt-4 d-flex align-items-center">
          ...
          />
          <Form.Label style={{ marginRight: 15 }}> مدیر</Form.Label>
          ...
          />
          <Form.Label style={{ marginRight: 15 }}> مدیر ارگان</Form.Label>
...
export default AddManagerAsAdmin;
