...
const AddManager = ({ selectObjs }) => {
  ...
  const fetchManagerInfo = useCallback(async () => {
    try {
      const data = await getManagerInitInfo();
      setInitValues(data);
    } catch (error) {
  ...
  const [canPassChange, setCanPassChange] = useState(false);
  ...
  useEffect(() => {
    if (watch("serviceCoordinate") && showServiceModal) {
      getLocationInfo(
  ...
  useEffect(() => {
    if (selectObjs) {
      const handlesetDefaultValues = (key, value) => {
        switch (key) {
          case "organNameId":
            const organName = selectObjs.organNameOptions?.find(
              (o) => o.value == value
            );
            return organName;
          case "eduDegreeId":
  ...
  }, [initValues, selectObjs]);

  const coordToString = (coord) => `{"lat":${coord.lat},"lng":${coord.lng}}`;
  const selectToValue = (select) => select.value;

  const changePassConfirmOtp = async () => {
    try {
      await sendMeChangePassOtpCode();
      setResendShow(false);
      setChangingPass(true);
      openOtpModal();
    } catch (error) {
      const elseError =
        "مشکلی در ارسال پیامک احراز هویت جهت تغییر نام کاربری و رمز عبور ایجاد شده است";
      errorHandler(router, error, elseError);
    }
  };

  const handelPassChangeCnfrmCode = async () => {
    if (!otp || otp.length != 5) {
      toast.error("لطفا کد 5 رقمی دریافتی را به درستی وارد نمایید");
      return;
    }
    try {
      await confirmCanChangePass(otp);
      toast.success("حال می توانید فرم را ارسال کنید");
      closeOtpModal();
      setCanPassChange(true);
      setChangingPass(false);
    } catch (error) {
      const elseError =
        "مشکلی در تایید کد ارسال شده برای تغییر نام کاربری و رمز عبور ایجاد شده است";
      errorHandler(router, error, elseError);
    }
  };

  const onSubmit = async (data) => {
    if (data["password"] && initValues["phoneConfirmed"] != true) {
      toast.error("برای تغییر پسورد نیاز به تایید شماره موبایل است");
      return;
    }

    if (
      (data["password"] || data["userName"] != initValues["userName"]) &&
      initValues["phoneConfirmed"] == true
    ) {
      if (!canPassChange) {
        toast.warning(
          `برای تغییر نام کاربری و رمز عبور،  نیاز به احراز هویت با پیامک است`
        );
        await changePassConfirmOtp();
        return;
      }
    }
    ...
    //generalize coordinates
    if (
      data["birthCoordinate"] != initValues["birthCoordinate"] ||
      !initValues["publicBirthCoordinate"]
    ) {
      const addrss =
    ...
    const dataToSend = {
      ...
      organNameId: selectToValue(watch("organName")),
    ...
    try {
      await submitManagerInfo(dataToSend);
      ...
      if (canPassChange) {
        ...
        toast.success(
          "بدلیل تغییر رمز عبور، نیاز است دوباره وارد حساب کاربری خود شوید"
        );

        router.push("/auth/login");
      } else {
        setChangingPass(false);
        setCanPassChange(false);
        reset();
  ...
  const handelConfirmMobile = async () => {
    try {
      await sendMeOtpCode();
      setResendShow(false);
      openOtpModal();
    } catch (error) {
      const elseError = "مشکلی در ارسال پیامک تایید شماره موبایل ایجاد شده است";
      errorHandler(router, error, elseError);
    }
  };

  const handelConfirmOtpCode = async () => {
    if (!otp || otp.length != 5) {
      toast.error("لطفا کد 5 رقمی دریافتی را به درستی وارد نمایید");
      return;
    }
    try {
      await confirmMyPhone(otp);
      await fetchManagerInfo();
      toast.success("شماره موبایل شما با موفقیت تایید شد");
      closeOtpModal();
    } catch (error) {
      const elseError = "مشکلی در تایید شماره موبابل ایجاد شده است";
      errorHandler(router, error, elseError);
    }
  };
  ...
  return (
    ...
    >
      {initValues["managerConfirmed"] == true && (
        <Alert variant="warning">
          اطلاعات شما تاییده شده است، ویرایش آن در صورت لزوم به وضعیت "نیاز به
          تایید دوباره"تبدیل خواهد شد
        </Alert>
      )}
      <Row className="my-4">
        <Col md={4}>
        ...
        <Col md={4}>
          <Form.Group>
            <Form.Label>تغییر رمز عبور</Form.Label>
            <Form.Control
              type="text"
          ...
          <Form.Group>
            ...
            {initValues["mobile"] && initValues["phoneConfirmed"] != true && (
              <Form.Text className="mx-2 text-danger">
                (لطفا شماره موبایل را با ارسال کد 5 رقمی تایید نمایید)
              </Form.Text>
            )}
          ...
          >
              ...
              <Container>
                <Row>
                  <Col xs={12} md={8}>
                    <Form.Group>
                      <Form.Text>
                        کد تایید 5 رقمی ارسال شده را وارد نمایید:
                      </Form.Text>
                      <OTPInput
                        value={otp}
                        onChange={setOtp}
                        isDisabled={resendShow}
                        containerStyle={{
                          display: "flex",
                    ...
                    {resendShow ? (
                      ...
                      <div style={{ marginTop: 20 }}>
                        <CountdownCircleTimer
                          isPlaying
                          duration={60}
                          colors={["#03b1fc", "#8403fc", "#fcba03", "#f50000"]}
                          colorsTime={[60, 45, 10, 0]}
                          size={120}
                          onComplete={() => {
                            setResendShow(true);
                          }}
            ...
            <Modal.Footer style={{ direction: "ltr" }}>
              ...
              <Button
                ...
                onClick={
                  changingPass
                    ? handelPassChangeCnfrmCode
                    : handelConfirmOtpCode
                }
                className="mx-1"
              >
                ثبت
      ...
      <Row>
        ...
        <Col md={3}>
          <Form.Group>
            ...
            <Form.Label>نام دستگاه</Form.Label>
            <Controller
              control={control}
              name="organName"
              rules={{ required: true }}
              render={({ field }) => (
                <Select
                  {...field}
                  options={selectObjs.organNameOptions}
                  placeholder="انتخاب کنید"
                  isSearchable={true}
                  isClearable={true}
                  isRtl={true}
                  defaultValue={watch("organName")}
                  noOptionsMessage={() => "یافت نشد"}
                />
              )}
            />
            {/* //TODO: Make Select a component */}
          </Form.Group>
        </Col>
        <Col md={3}>
          <Form.Group>
